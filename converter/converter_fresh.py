...
file = open("test_record.txt","a+")
s = mmap.mmap(file.fileno(), 0, access=mmap.ACCESS_READ)

#for row in cur.fetchmany(27):
for row in cur.fetchall():

    #Insert if else into here that checks the combo of ID, ambulation number, and date against the existing txt file
    #and only does this stuff if its new 
    identifier = str(row[0]) + ',' + str(row[1]) + ',' + str(row[2])
    if s.find(str.encode(identifier)) == -1:
